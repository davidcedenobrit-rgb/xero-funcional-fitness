      ...
      <div className="relative w-full max-w-md px-6 animate-fade-in">
        ...
        <div className="text-center mb-10">
          <div className="flex justify-center mb-6">
            <img src="/xero-logo.png" alt="XERO Funcional Fitness" className="h-16 object-contain" />
          </div>
        ...
        <div className="bg-[#141614] border border-[#1E261E] rounded-2xl p-8">
          <div className="flex items-center gap-2 mb-6">
            <div className="w-1.5 h-4 bg-[#5B8A3C] rounded-full" />
            <span className="text-sm text-[#6B7A6B] uppercase tracking-widest">Acceso Seguro</span>
          </div>

          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              <label className="block text-sm text-[#6B7A6B] mb-2 uppercase tracking-wider">Email</label>
              <input
                type="email" value={email} onChange={e => setEmail(e.target.value)} required
                placeholder="admin@xerofitness.com"
            ...
            <div>
              <label className="block text-sm text-[#6B7A6B] mb-2 uppercase tracking-wider">Contraseña</label>
              <div className="relative">
                <input
                  type={showPassword ? 'text' : 'password'} value={password}
                  onChange={e => setPassword(e.target.value)} required placeholder="••••••••"
                  className="w-full bg-[#0A0C0A] border border-[#1E261E] rounded-xl px-4 py-3 text-white placeholder-[#444] focus:outline-none focus:border-[#5B8A3C] transition-colors pr-12"
                />
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-[#5B8A3C] hover:bg-[#4A7230] disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-colors uppercase tracking-widest text-sm"
        ...
        <p className="text-center text-[#333] text-xs mt-6">
          © {new Date().getFullYear()} XERO Funcional Fitness · Todos los derechos reservados
        </p>
      </div>
    </div>
  )
